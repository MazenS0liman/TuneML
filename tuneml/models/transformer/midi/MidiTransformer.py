...
class MidiDecoderLayer(nn.Module):
    ...
    """
    def __init__(self, d_model, num_heads, d_ff, bias=True, dropout=0.1, layernorm_eps=1e-6):
        super().__init__()
    ...
    def forward(
        ...
        x = self.norm1(tgt)
        x = tgt + self.drop1(self.self_attn(x, x, x, mask=self_attn_mask))

        # 2. cross-attention: MIDI queries attend to text
        y = self.norm2(x)
        y = x + self.drop2(self.cross_attn(y, memory, memory, mask=cross_attn_mask))
